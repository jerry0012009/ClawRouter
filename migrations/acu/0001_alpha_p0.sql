...
CREATE TABLE acu_judge_evaluations (
  ...
  explanation TEXT,
  web_intent TEXT CHECK (web_intent IN ('required', 'likely', 'not_required')),
  web_intent_confidence DOUBLE PRECISION CHECK (web_intent_confidence BETWEEN 0 AND 1),
  web_intent_reason TEXT,
  web_intent_evidence_json JSONB NOT NULL DEFAULT '[]'::jsonb,
  web_intent_source TEXT CHECK (web_intent_source IN ('judge', 'heuristic_fallback', 'legacy_heuristic')),
