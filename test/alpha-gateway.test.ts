...
describe("Alpha native protocol gateway", () => {
  it("serves an authenticated zero-call selection corridor projection", async () => {
    ...
    const gatewayPort = await listen(
      createAlphaGatewayServer({
        ...
        async resolveExecution() {
          throw new Error("corridor projection must not route an execution");
        },
      }),
    );
    const unauthorized = await fetch(
      `http://127.0.0.1:${gatewayPort}/internal/admin/selection-corridor`,
    );
    ...
    expect(calls).toEqual([
      { inputTokens: 24000, expectedOutputTokens: 1200, protocol: "messages" },
    ]);
  });
  ...
  it("protects Channel Monitor and records an authorized manual pause", async () => {
      ...
      createAlphaGatewayServer({
        ...
        adminChannelMonitor: {
          ...
          async load(query, utilityPolicy) {
            ...
            monitorPolicies.push(utilityPolicy);
        ...
        async resolveExecution() {
          throw new Error("monitor must not route an execution");
        },
      }),
    );
    const unauthorized = await fetch(
      `http://127.0.0.1:${gatewayPort}/internal/admin/channel-monitor`,
    );
    expect(unauthorized.status).toBe(401);
      ...
      `http://127.0.0.1:${gatewayPort}/internal/admin/channel-monitor?range=6h&supplyStrategy=low_latency&scenario=small`,
      {
        headers: {
          ...
          "x-acu-monitor-routing-utility-policy": JSON.stringify({
            ...
            profileCostLogScale: 7,
          }),
        },
      },
    );
    expect(monitor.status).toBe(200);
    expect(monitorQueries).toEqual([
      { range: "6h", supplyStrategy: "low_latency", scenario: "small" },
    ]);
    ...
    expect(pauses).toEqual([
      { channelId: "lucen-cx014-pro-stable", durationMinutes: 30, actor: "admin-fixture" },
    ]);
  ...
  it("returns verifiable build, migration, Judge, and routing identity in health", async () => {
    ...
    const gatewayPort = await listen(
      createAlphaGatewayServer({
        ...
        async resolveExecution() {
          throw new Error("health must not route an execution");
        },
      }),
    );
  ...
  it("recognizes only loopback and private network ingress addresses", () => {
    expect(
      [
        ...
        "192.168.1.2",
        "fd00::1",
      ].every(isPrivateNetworkAddress),
    ).toBe(true);
    expect(
      ["8.8.8.8", "172.15.0.1", "172.32.0.1", "2001:4860:4860::8888"].some(isPrivateNetworkAddress),
    ).toBe(false);
  });
  ...
  it("protects full traces with an independent administrator bearer token", async () => {
    ...
    const gatewayPort = await listen(
      createAlphaGatewayServer({
        ...
        async resolveExecution() {
          throw new Error("admin traces must not resolve an execution");
        },
      }),
    );

    const unsigned = await fetch(
      `http://127.0.0.1:${gatewayPort}/internal/admin/traces/req_trace_1`,
    );
    expect(unsigned.status).toBe(401);
    ...
    const absent = await fetch(
      `http://127.0.0.1:${gatewayPort}/internal/admin/traces/req_missing`,
      {
        headers: { authorization: "Bearer independent-admin-token-not-production" },
      },
    );
  ...
  it("treats a protocol-complete SSE response as completed when the client closes immediately", async () => {
    ...
    expect(traces.at(-1)).toMatchObject({
      status: "completed",
      response: { complete: true, clientCancelled: false, protocolCompleted: true },
    });
  });
      ...
      createAlphaGatewayServer({
        ...
        async resolveExecution(envelope) {
          return {
            adapter: {
              async execute() {
                return new Response('{"id":"large-fixture","output":[]}');
              },
            },
            requestedModel: envelope.requestedModel,
            actualModel: envelope.requestedModel,
  ...
  it("preserves Messages tool_use and thinking signature bytes", async () => {
    ...
    const rawBody = {
      ...
      messages: [
        {
          role: "assistant",
          content: [{ type: "thinking", thinking: "private", signature: "signed" }],
        },
      ],
      tools: [
        { name: "future_tool", input_schema: { type: "object", future_schema_keyword: true } },
      ],
  ...
  it("adds the native Messages version only when absent", async () => {
    const seen: Array<string | undefined> = [];
    const upstreamPort = await listen(
      createServer(async (request, response) => {
        seen.push(request.headers["anthropic-version"] as string | undefined);
        response.setHeader("content-type", "text/event-stream");
    ...
    const body = Buffer.from('{"model":"m","messages":[],"stream":true}');
    ...
    expect(seen).toEqual(["2023-06-01", "custom-version", undefined]);
  });

  it.each([
    [400, "thinking is not supported"],
  ...
  it("uses a native Messages error envelope for invalid trusted identity", async () => {
    ...
    expect(await response.json()).toEqual({
      ...
      error: {
        type: "api_error",
        message: "Missing or repeated trusted identity header: x-acu-newapi-user-id",
      },
    });
  });
