...
export function usageFinalizeBody(report: PendingUsageReport): Buffer {
  return Buffer.from(JSON.stringify({
    ...
    judge_input_tokens: safeTokenCount(report.judgeInputTokens ?? 0n),
    judge_output_tokens: safeTokenCount(report.judgeOutputTokens ?? 0n),
    judge_official_payg_equivalent_cost: report.judgeOfficialPaygEquivalentCost ?? "0.0000000000",
    judge_cost_currency: report.judgeCostCurrency ?? "CNY",
    judge_cost_status: report.judgeCostStatus ?? "not_applicable",
    judge_cost_source: report.judgeCostSource ?? "not_applicable",
    judge_provider: report.judgeProvider,
